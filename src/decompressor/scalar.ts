import {
    ...
    REGEX_UNREFERENCED_ESCAPED_STRING_TOKEN,
    DATE_LOW_PRECISION,
    ...
    NULL_TOKEN,
    UNDEFINED_TOKEN,
    REGEX_ESCAPED_STRING_TOKEN,
    INTEGER_SMALL_TOKEN_EXCLUSIVE_BOUND_LOWER,
    INTEGER_SMALL_TOKEN_EXCLUSIVE_BOUND_UPPER,
    INTEGER_SMALL_TOKEN_OFFSET,
    REGEX_ESCAPED_ESCAPE_CHARACTER,
} from '../constants.ts'
import { decompressInteger, decompressFloat } from '../util.ts'
import {
    SKIP_SCALAR,
    type OrderedIndex,
    type Cursor,
...
/**
 * Decompress a scalar token starting at the cursor position.
 * 从当前游标位置解压一个标量令牌。
 *
 * @param token - Current token character.
 * @param token - 当前令牌字符。
 ...
 * @returns Decoded scalar value or SKIP_SCALAR if more data is needed.
 * @returns 解码后的标量值，或需要更多数据时返回 SKIP_SCALAR。
 */
export function decompressScalar(
    token: string,
    data: string,
    cursor: Cursor,
    ...
    if (
        ...
        while (
            !(data.charCodeAt(endIndex) > DELIMITING_TOKENS_THRESHOLD) &&
            endIndex < data.length
        ) {
            endIndex++
        }
    ...
    if (
        tokenCharCode > INTEGER_SMALL_TOKEN_EXCLUSIVE_BOUND_LOWER &&
        tokenCharCode < INTEGER_SMALL_TOKEN_EXCLUSIVE_BOUND_UPPER
    ) {
        ...
        return (orderedIndex.strings[orderedIndex.strings.length] = data
            .substring(startIndex + 1, endIndex - 1)
            .replace(REGEX_ESCAPED_ESCAPE_CHARACTER, ESCAPE_CHARACTER)
            .replace(REGEX_ESCAPED_STRING_TOKEN, STRING_TOKEN))
    } else if (token === INTEGER_TOKEN) {
        return (orderedIndex.integers[orderedIndex.integers.length] = decompressInteger(
            data.substring(startIndex + 1, endIndex),
        ))
    } else if (token === FLOAT_TOKEN) {
        return (orderedIndex.floats[orderedIndex.floats.length] = decompressFloat(
            data.substring(startIndex + 1, endIndex),
        ))
    } else if (token === DATE_TOKEN) {
        return (orderedIndex.dates[orderedIndex.dates.length] = new Date(
            decompressInteger(data.substring(startIndex + 1, endIndex)),
        ).toISOString())
    } else if (token === LP_DATE_TOKEN) {
        return (orderedIndex.lpDates[orderedIndex.lpDates.length] = new Date(
            DATE_LOW_PRECISION * decompressInteger(data.substring(startIndex + 1, endIndex)),
        ).toISOString())
    } else if (token === UNREFERENCED_STRING_TOKEN) {
        return data
            .substring(startIndex + 1, endIndex - 1)
            .replace(REGEX_ESCAPED_ESCAPE_CHARACTER, ESCAPE_CHARACTER)
            .replace(REGEX_UNREFERENCED_ESCAPED_STRING_TOKEN, UNREFERENCED_STRING_TOKEN)
